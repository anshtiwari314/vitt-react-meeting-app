...
export default function DataWrapper({children}:{children:React.ReactNode}) {
    ...
    let usersFlag = useRef(false)
    let users2Flag =  useRef(false)
    ...
    useEffect(()=>{
        let id = setInterval(()=>{
            if(tempData.current.length>0 && tempData2.current.length){
                
                usersArrRef.current = [...usersArrRef.current]
                setUsers(prev=>[...prev,...tempData.current,...tempData2.current])
            }else if(tempData.current.length>0){

                setUsers(prev=>[...prev,...tempData.current])
            }else if(tempData2.current.length){

                setUsers(prev=>[...prev,...tempData2.current])
            }
    ...
    useEffect(()=>{
        ...
        gettingMediaStream().then(bothStreams=>{
            ...
            usersArrRef.current.push({id:myId,stream:bothStreams,isAdmin:false,isAudioStream:false,isLoading:false})
            setUsers([
            ...
            gettingAudioStream().then(audioStream=>{
                ...
                usersArrRef.current.push({id:myId,stream:audioStream,isAdmin:false,isAudioStream:true,isLoading:false})
                setUsers([
    ...
    useEffect(()=>{
        ...
        function timeOut(){
            ...
            tempData2.current = [...tempData2.current,...users2Ref.current]
            users2Ref.current = []
            },2000)
        }
        ...
        function reply(call:any){
            ...
            socket.emit('connected-user-data',{...usersArrRef.current[0],toPeer:call.peer,isLoading:true,toReceiver:false})
            ...
            call.on('stream',(userVideoStream:MediaStream)=>{
                ...
                if(!peersArrRef.current.includes(call.peer)){
                    ...
                    for(let i=0;i<users2Ref.current.length;i++){
                        ...
                        console.log('tempuser 2',users2Ref.current)
    ...
    useEffect(()=>{
        ...
        function timeOut(){
            ...
            tempData.current = [...tempData.current,...usersRef.current]
            usersRef.current = [] 
                        ...
                        //let tempUser = initialUser
                           ...
                           // let matched=usersRef.current.filter((e,i)=>myId ===e.toPeer)
                            ...
                            for(let i=0;i<usersRef.current.length;i++){
                                ...
                                console.log('tempuser 1',usersRef.current)
                                // usersRef.current.splice(i,1)
